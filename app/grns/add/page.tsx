...
import { createGRN, getStores, getWorkLogs, getVehicles, type Store, type WorkLog, type Vehicle } from "@/lib/api";
...
export default function AddGRNPage() {
  ...
  useEffect(() => {
    ...
    const loadReferences = async () => {
      try {
        const [storeItems, worklogItems, vehicleItems] = await Promise.all([
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setPageError(error instanceof Error ? error.message : "Unable to save GRN");
      setSaving(false);
    }
  };
  ...
  return (
    <AppShell
      active="grns"
      heading="Add Goods Receipt Note"
      description="Log incoming coconuts into a store/hub."
